...
public class UserService {
	...
	public User getCurrentUser() {
		String email = SecurityUtils.getCurrentUserLogin()
				.orElseThrow(() -> new ResourceNotFoundException(ErrorMessage.PRINCIPAL_NOT_FOUND_MESSAGE));
	...
	public void updatePassword(UpdatePasswordRequest updatePasswordRequest) {
		...
		// if the current entered password is same with the old password
		if (passwordEncoder.matches(updatePasswordRequest.getOldPassword(), user.getPassword())) {
			throw new BadRequestException(ErrorMessage.PASSWORD_NOT_MATCHED);
		}
		// to encode new password
	...
	@Transactional// Specifically for sql query with more than one fields, it doesn't let done without a blank field
	...
	public void updateUserAuth(Long id, AdminUserUpdateRequest adminUserUpdateRequest) {
		...
		//password is empty?
		if(adminUserUpdateRequest.getPassword().isBlank()) {
		...
		user.setPostCode(adminUserUpdateRequest.getPostCode());
