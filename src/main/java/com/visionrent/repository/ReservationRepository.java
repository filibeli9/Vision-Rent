...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
	...
	@EntityGraph(attributePaths = {"car","car.image"})//only bring car and car image not imageData
	List<Reservation> findAll();
	
	@EntityGraph(attributePaths = {"car","car.image"})
	Page<Reservation> findAll(Pageable pageable);
	
	@EntityGraph(attributePaths = {"car","car.image","user"})//to get car, car.image, user information with reservation
